...
# Página de registro de invitados
st.set_page_config(page_title="Registro para la Boda")
...
if st.sidebar.button("Iniciar sesión"):
    if usuario == "admin" and contrasena == "admin123":
        ...
        num_invitados = len(invitados)
        st.markdown(f"### Hay hasta el momento {num_invitados} asistentes.")
        
        if invitados:
            for invitado in invitados:
                id_invitado, nombre = invitado
                st.write(f"{nombre}")
                if st.button(f"Eliminar {nombre}", key=id_invitado):
